...
func runCreate(args []string) {
	...
	fs.StringVar(&p.SoftwareRepo, "software-repo", "", "owner/repo (on github.com) the bootstrap script git-clones and builds mseg-tester from -- no release needed (required)")
	...
	fs.StringVar(&p.SoftwareRef, "module-ref", "", "git branch/tag/commit the bootstrap script's local checkout (and every later self-update) tracks -software-repo at. Defaults to \"main\". Point at your own branch or a commit SHA to exercise unreleased code -- no GitHub release or build pipeline needed (\"test without gh\")")
